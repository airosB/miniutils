...
function convert(id) {
	...
	var instr = gebi("input_string").value
	init = new Array();
	init = instr.split("");

	var ncc = 0; //number of converted characters
	var outstr = "";


	for (var i = 0; i <= init.length+5; i++) {

		if ( rtoh1[ init[ncc+0] ] ) {
			outstr = outstr + rtoh1[ init[ncc+0] ];
			ncc++;
		} else if ( skip[ init[ncc+0]+init[ncc+1] ] ) {
			outstr = outstr + skip[ init[ncc+0]+init[ncc+1] ];
			ncc = ncc + 1;
		} else if ( rtoh2[ init[ncc+0]+init[ncc+1] ] ) {
			outstr = outstr + rtoh2[ init[ncc+0]+init[ncc+1] ];
			ncc = ncc + 2;
		} else if ( rtoh3[ init[ncc+0]+init[ncc+1]+init[ncc+2] ] ) {
			outstr = outstr + rtoh3[ init[ncc+0]+init[ncc+1]+init[ncc+2] ];
			ncc = ncc + 3;
		} else if ( rtoh4[ init[ncc+0]+init[ncc+1]+init[ncc+2]+init[ncc+3] ] ) {
			outstr = outstr + rtoh4[ init[ncc+0]+init[ncc+1]+init[ncc+2]+init[ncc+3] ];
			ncc = ncc + 4;
		} else if ( init.length - 5 >= ncc ) {
			outstr = outstr + init[ncc+0];
			ncc++;
		}
	}

	if ( ! gebi('oldtype').checked ) {
		outstr = outstr.replace(/ゐ/g,"うぃ");
		outstr = outstr.replace(/ゑ/g,"うぇ");
	}


	gebi("converted").value = ncc;
	gebi("output_string").value = outstr;
}
